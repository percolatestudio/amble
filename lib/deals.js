...
Deals.findNearby = function(latLng, maxDistance, options) {
  options = options || { limit: 10, fields: { metadata: 0 } };
...
Deals.findSaved = function(userId, options) {
  options = options || { fields: { metadata: 0 }};
  var user = Meteor.users.findOne(userId);
  if (!user) {
    throw new Meteor.Error('invalid-user', "That user doesn't exist");
  }

  return Deals.find({ _id: { $in: user.profile.savedDeals || [] }}, options);
};
