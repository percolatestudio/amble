...
Deals.findNearby = function(userId, latLng, maxDistance, options) {
  var options = options || { limit: 10 };
...
Deals.findNearest = function(userId, latLng, maxDistance) {
  return Deals.findNearby(userId, latLng, maxDistance, {limit: 1}).fetch()[0];
};
...
Deals.findSaved = function(userId) {
  ...
  return Deals.find({ userId: userId, _id: { $in: user.profile.savedDeals }});
};
...
Deals.mutate = {
  upsert: function(doc) {
    ...
    // XXX
    // This is probably not unique enough to ensure the upsert?
